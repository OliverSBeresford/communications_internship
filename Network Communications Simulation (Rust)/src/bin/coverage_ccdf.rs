...
fn main() {
    ...
    let base_station_density = if args.len() > 1 {
        args[1].parse::<f64>().unwrap_or_else(|_| {
            eprintln!("Usage: {} [base_station_density]", args[0]);
            eprintln!("Using default density of 20.0 BS per km^2");
            20.0
        })
    } else {
        eprintln!("Usage: {} [base_station_density]", args[0]);
    ...
    // Output CSV files for NLOS data
